...
pub fn compute_opacity(
    ...
    let timeout = if is_pip { 2.0 } else { 5.0 };
    let controls_target = if playing && idle_secs > timeout { 0.0 } else { 1.0 };
    let center_target = if playing && idle_secs > timeout { 0.0 } else if playing { 0.0 } else { 1.0 };

    let new_ctrl = if controls_target > cur_ctrl {
        1.0 // Instant fade in
    } else {
        (cur_ctrl + (controls_target - cur_ctrl) * 0.08).clamp(0.0, 1.0) // Smooth fade out
    };
